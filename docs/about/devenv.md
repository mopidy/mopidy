...
Mopidy install, as described in installation. That is, if you're running
...
`/mopidy-dev/mopidy/src/mopidy/` Python package directory.
...
extensions so that Mopidy may find them through `importlib`.
The result of this can be seen in a file named `entry_points.txt`
which can be found inside the virtualenv dir, e.g.
`/mopidy-dev/.venv/lib/python3.13/site-packages/Mopidy-4.0.0.dist-info/entry_points.txt`.
The `entry_points.txt` file is of special interest as it shows both how
the above executable and the bundled extensions are connected to the Mopidy
source code:

```ini title="entry_points.txt"
[console_scripts]
mopidy = mopidy._app.cli:app.meta

[mopidy.ext]
file = mopidy._exts.file:Extension
http = mopidy._exts.http:Extension
m3u = mopidy._exts.m3u:Extension
softwaremixer = mopidy._exts.softwaremixer:Extension
stream = mopidy._exts.stream:Extension
```
...
includes are defined in `pyproject.toml`. Since pip v25.1 you can
instead do:
...
If you take a look at the tox config in `pyproject.toml`, you'll see that tox
runs tests in multiple environments, including `ruff-check` and `ruff-format`
environments that lint the source code for issues and a `docs` environment
that tests that the documentation can be built. You can also limit tox to just
test specific environments using the `-e` option, e.g. to run just unit tests
on Python 3.13:
...
To learn more, see the [tox documentation](https://tox.readthedocs.io/) .
...
Under the hood, `tox -e 3.13` will use [pytest](https://docs.pytest.org/)
...
[Codecov](https://codecov.io/gh/mopidy/mopidy).
///
...
Mopidy uses [GitHub Actions](https://github.com/mopidy/mopidy/actions) for
...
[Codecov](https://codecov.io/gh/mopidy/mopidy). If you're out of work,
Codecov might help you find areas in the code which could need better test
coverage.

### Style checking and linting

We're quite pedantic about codestyle and try hard to keep the Mopidy
code base a very clean and nice place to work in.

Luckily, you can get very far by using the [ruff](https://github.com/astral-sh/ruff) linter to check your code for issues before
submitting a pull request. Mopidy's ruff rules are configured in `pyproject.toml`.
You can either run the `ruff-check` tox environment, like our CI setup will do
...
We use [Zensical](https://zensical.org/) to build the documentation site.

To serve the documentation locally with live reloading, run:
...
The documentation at <https://docs.mopidy.com/> is automatically updated
...
Before you being, make sure you've read the contributing page and the
...
`/mopidy-dev/mopidy/`, we can run `git remote -v` to list the
...
If you haven't already, [fork the repository](https://help.github.com/en/articles/fork-a-repo) to your own GitHub account.
...
commit messages. All as described in contributing and elsewhere in
the devenv guide.
...
<https://github.com/mopidy/mopidy>, and [create a pull request](https://help.github.com/en/articles/creating-a-pull-request).
...
See running-tests on how to run the same tests locally as
