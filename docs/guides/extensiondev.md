...
We've made a [Copier](https://copier.readthedocs.io/) project template for
[creating new Mopidy extensions](https://github.com/mopidy/mopidy-ext-template).
...
See the readme of [mopidy-ext-template](https://github.com/mopidy/mopidy-ext-template) for further details.
...
The extension must be registered on [PyPI](https://pypi.org/). The name of the
...
For Mopidy to find your extension, the extension must register an [entry point](https://packaging.python.org/en/latest/specifications/entry-points/) of the
`mopidy.ext` type.

This is done by adding a section like this to your
`pyproject.toml` file:

```toml title="mopidy-soundspot/pyproject.toml"
...
inherits from Mopidy's extension base class, `mopidy.ext.Extension`. This
...
imported without raising `ImportError` exceptions for missing dependencies,
etc.
...
`ConfigParser` compatible config section. The config section's name must be
...
```ini title="mopidy-soundspot/src/mopidy_soundspot/ext.conf"
[soundspot]
enabled = true
username =
password =
...
For more detailed documentation on the extension class, see the ext-api.
...
[Frontend API](../reference/frontend.md) for more details.

```python title="mopidy-soundspot/src/mopidy_soundspot/frontend.py"
import pykka
...
The skeleton of a backend would look like this. See the [Backend API](../reference/backend.md) for more
details.

```python title="mopidy-soundspot/src/mopidy_soundspot/backend.py"
import pykka
...
See the [Cyclopts documentation](https://cyclopts.readthedocs.io/) for more
details.

```python title="mopidy-soundspot/src/mopidy_soundspot/commands.py"
import cyclopts
...
[HTTP server API](../reference/http-server.md) docs or explore with the
[mopidy-api-explorer](https://mopidy.com/ext/api-explorer) extension.

## Python conventions

In general, it would be nice if Mopidy extensions followed the same
codestyle as Mopidy itself, as they're part of the same ecosystem.

## Use of Mopidy APIs

When writing an extension, you should only use APIs documented in the
[Reference](../reference/index.md) section. Any package or module whose name
starts with an underscore (e.g. `mopidy._internal`) is considered private, may
change at any time, and should not be used by extensions.
...
going on. We use the `logging` module from Python's standard library. When
...
If you make HTTP requests please make sure to respect the [proxy
config](../usage/config.md#proxy-section), so that all the requests you make go
through the proxy configured by the Mopidy user. To make this easier for
extension developers, the helper function [mopidy.httpclient.format_proxy][] was
added in Mopidy 1.1. This function returns the proxy settings [formatted the way
Requests expects](https://requests.readthedocs.io/en/latest/user/advanced/#proxies).
...
added the helper function [mopidy.httpclient.format_user_agent][]. Here's
...
Most Mopidy extensions that make HTTP requests use either the
[HTTPX](https://www.python-httpx.org/) or
[Requests](https://requests.readthedocs.io/) library to do so.
...
For further details, see HTTPX' docs on [clients](https://www.python-httpx.org/advanced/clients/).
...
For further details, see Requests' docs on [session objects](https://requests.readthedocs.io/en/latest/user/advanced/#proxies).
...
Python [mock library](https://docs.python.org/dev/library/unittest.mock.html).

When it comes to running tests, Mopidy typically makes use of testing tools
like [tox](https://tox.readthedocs.io/) and
...
```python title="mopidy-soundspot/tests/test_extension.py"
...
- [pytest-httpx](https://pypi.org/project/pytest-httpx/) - A pytest plugin for
  mocking HTTPX requests.
- [responses](https://pypi.org/project/responses) - A utility library for
  mocking out the Requests Python library.
- [vcrpy](https://pypi.org/project/vcrpy) - Automatically mock your HTTP
...
[Pykka API](https://pykka.readthedocs.io/) at this point to make sure that
...
You'll also need a list of `mopidy.models.Track` and a list of URIs in
...
[/tests](https://github.com/mopidy/mopidy/tree/main/tests)
