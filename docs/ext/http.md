...
[`http/port`](#httpport) config value.
...
[`http/hostname`](#httphostname) config value. Before you do so, note that the HTTP
...
client. See [HTTP server API](../reference/http-server.md) for how to make
static files or server-side functionality from a Mopidy extension available
through Mopidy's web server.
...
See [HTTP API](../reference/http.md) for details on how to integrate with
Mopidy over HTTP. If you're looking for a web based client for Mopidy, go
check out [Clients](../usage/clients.md).
...
requests. This applies to both JSON-RPC and WebSocket requests.

Values should be in the format `hostname:port`, should not specify any scheme
and be separated by either a comma or newline. Additionally, the `port` should
...
checks, effectively ignoring the [`http/allowed_origins`](#httpallowed_origins) config
...
points. By convention, this is the extension's `ext_name`.
